...
class Server:
    ...
    def start_db(self):
        ...
        for i in range(max_retries):
            try:
                db = psycopg2.connect(database=self.db_name, user=self.user, password=self.pasword, host=self.host, port="5432")
                cursor = db.cursor()
                ...
                print(f"Попытка подключения к базе данных {i+1}/{max_retries}... Ожидание {retry_interval} секунд")
    ...
    def start_server(self, conn=5, buffer=4096, port=8080):
        try:
            ...
            time.sleep(.5)
                ...
                print(self.getTimeStampp() + "  Error: Cannot start server..." + str(e))
                self.write_log(self.getTimeStampp() + "  Error: Cannot start server..." + str(e))
